...
def AddAngularDist():
    ...
    sections41 = tape21.material(tape20.material_numbers[0]).file(4).sections.to_list()

    section4 = sections41[0].parse()

    # Extract angular distribution data (could be Legendre or Tabulated)
    distribution1 = section4.angular_distributions[:]  # Updated data
    distribution0 = sections40[0].angular_distributions[:]  # Original data

    # Store interpolation information from the updated file
    main = sections41[0].parse().distributions
    ...
    new_legendre = ENDFtk.MF4.LegendreDistributions([len(legendre_distributions)], main.interpolants.to_list(), legendre_distributions)

    if len(tabulated_distributions) != 0:
        if tabulated_distributions[0].E != legendre_distributions[-1].E:
            tabulated_distributions = [ENDFtk.MF4.TabulatedDistribution(legendre_distributions[-1].E, tabulated_distributions[0].boundaries, tabulated_distributions[0].interpolants, tabulated_distributions[0].cosines, tabulated_distributions[0].probabilities)] + tabulated_distributions
        
        new_tabulated = ENDFtk.MF4.TabulatedDistributions([len(tabulated_distributions)], main.interpolants.to_list(), tabulated_distributions)
    ...
    sections_new = ENDFtk.MF4.Section(sections40[0].MT, sections40[0].ZA, sections40[0].AWR, sections40[0].LCT, combined)
